...
<div class="mb-3 d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-3">
    <form method="GET" class="d-flex w-100 w-md-auto">
        <div class="input-group">
            ...
            <input type="text" name="search" value="{{ request('search') }}" class="form-control border-start-0" placeholder="Search sales...">
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="fw-bold">{{ $sale->product->name ?? '-' }}</span><br>
                        ...
                        <td class="text-end pe-4">
                            <a href="{{ route('sales.edit', $sale->id) }}" class="btn btn-sm btn-outline-primary me-2">
                                <i class="bi bi-pencil"></i>
                            </a>

                            <form action="{{ route('sales.destroy', $sale->id) }}" method="POST" class="d-inline-block" onsubmit="return confirm('Delete this sale?');">
                                @csrf
                                @method('DELETE')
                    ...
                    <tr>
                        <td colspan="6">
                            <div class="p-4 text-center">
                                ...
                                <p class="text-muted mb-3">Record your first sale to get started.</p>
                                <a href="{{ route('sales.create') }}" class="btn btn-primary btn-sm">Add Sale</a>
                            </div>
                        </td>
                    </tr>
                    @endforelse
...
<div class="d-flex justify-content-between align-items-center">
    <div class="text-muted small">
        Showing {{ $sales->firstItem() ?? 0 }} to {{ $sales->lastItem() ?? 0 }} of {{ $sales->total() ?? 0 }} sales
    </div>
    <div>
